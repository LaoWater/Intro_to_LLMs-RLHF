...
class RLHF_Exploration_Agent:
    def __init__(self, initial_exploration_rate=0.5, decay=0.95, grid_size=5, policy_file="agent_policy.pkl"):
        ...
        self.policy_file = policy_file  # File to save and load policy

        # Load existing policy if file exists
        self.load_policy()
    ...
    def print_policy(self):
        ...
        for state, action in self.policy.items():
            print(f"State: {state}, Action: {map_action_to_movement(action)}")
...
def rl_human_feedback_simulation(env, agent, steps=100):
    ...
    for step in range(steps):
        ...
        next_state, reward, done = env.step(action)  # Get next state after action
        ...
        print(f"Step: {step + 1}, Action: {map_action_to_movement(action)}, Feedback: {feedback}, Exploration Rate: {agent.exploration_rate:.2f} \n")
    ...
    # Print the learned policy at the end
    agent.print_policy()
...
if __name__ == "__main__":
    env = SimpleGridEnv()
    agent = RLHF_Exploration_Agent(initial_exploration_rate=0.5, decay=0.2, grid_size=5)
