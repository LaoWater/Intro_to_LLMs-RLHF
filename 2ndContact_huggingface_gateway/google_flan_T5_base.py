...
def run_flant5_cpu(input_text, max_new_tokens=50, temperature=0.7, top_k=50, repetition_penalty=1.2):
    """
    Runs the Flan-T5 Base model on CPU with specified generation parameters.
...
def run_flant5_gpu(input_text, max_new_tokens=150, temperature=0.7, top_k=50, repetition_penalty=1.2):
    """
    Runs the Flan-T5 Base model on GPU with specified generation parameters.
...
if __name__ == "__main__":
    ...
    print("\nRunning Flan-T5 Base on CPU:")
    run_flant5_cpu(input_text=input_prompt)

    # Check if CUDA (GPU) is available before attempting GPU execution
    if torch.cuda.is_available():
        print("\nRunning Flan-T5 Base on GPU:")
        gpu_generated_output = run_flant5_gpu(input_text=input_prompt)
