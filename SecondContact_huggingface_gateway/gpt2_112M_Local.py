...
def run_gpt2_cpu(input_text, max_new_tokens=50, temperature=0.7, top_k=50, repetition_penalty=1.2):
    """
    Runs the GPT-2 model on CPU with specified generation parameters.
    ...
    # GPT-2 does not have a pad token by default. Setting it to eos_token_id to avoid warnings.
...
def run_gpt2_gpu(input_text, max_new_tokens=150, temperature=0.7, top_k=50, repetition_penalty=1.2):
    """
    Runs the GPT-2 model on GPU with specified generation parameters.
...
if __name__ == "__main__":
    ...
    print("\nRunning GPT-2 on CPU:")
    run_gpt2_cpu(input_text=input_prompt)

    # Check if CUDA (GPU) is available before attempting GPU execution
    if torch.cuda.is_available():
        print("\nRunning GPT-2 on GPU:")
        gpu_generated_output = run_gpt2_gpu(input_text=input_prompt)
